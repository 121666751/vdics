/*  Config Class
 ...
 Modify: huangya(TCHZT), 25.09.2010 
         <adv@adv.org.ua>
         <t9r@mail.ru>  
	 <huangya@tchzt.com>*/
#include <memory.h>
...
int TilGetLine(FILE *rFile, std::string &rst)
{
	char	s[1024];
	memset(s, 0, sizeof(s));
	fgets(s, sizeof(s), rFile);
...
void
Value::set(std::string str, int bl, int key)
{
  value = str;
  type = key;
  block = bl;
...
int Config::read_config_file(void)
{
