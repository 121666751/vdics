...
#define CINPUT_PARSE_LOG		LOG_PREFIX"pisrv.log"
#define CINPUT_SEND_LOG			LOG_PREFIX"sisrv.log"
...
#define CINPUT_NODE					8
#define CINPUT_SEND_NODE			9
...
//		#define LogPrint(fmt, args...)	debug_print(stdout, __FILE__, __LINE__, __FUNCTION__, fmt, ##args);
		...
		#define LogPrint(node, format, args...) {\
			...
			else if (node == CINPUT_NODE)\
            	fd = fopen(CINPUT_PARSE_LOG, "a");\
			else if (node ==CINPUT_SEND_NODE)\
            	fd = fopen(CINPUT_SEND_LOG, "a");\
			if (fd != NULL) {\
