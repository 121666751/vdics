...
{
    CAnsiMemFile memFile;
    memFile.Write(body.orgcode,11,strlen(body.orgcode),true,' ');
...
int doTrade(const char* sends,const char *ip,const char* port,int  &result,char*response,char*outstr)
{
    ...
    LogPrint(VOUCHER_SEND_NODE, "接收数据[%s]\n",out);
    so.Close();
    ...
    strncpy(response,out+12,14);
    if(r!='N' && strlen(out)>26)
    {
        strncpy(outstr,out+26,100);
    }
    so.Close();
