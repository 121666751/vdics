...
//     int ret = _db.VQuery(512, "select fname,bname,pkgno,date,session,ip,rotate,dpi,vchno,vchtype,exchno,imageid,sid,clkno,area,(case when modeflag then 'N' else 'O' end) as modeflag from vouchers where sid=%s", sid);
	int ret = _db.VQuery(512, "select fname,bname,pkgno,date,session,ip ,rotate,dpi, concat(right(pkgno,6),'01',right(accno,10)) ,vchno,vchtype,right(exchno,6),amount*100,imageid,sid,clkno,area,(case when modeflag then 'N' else 'O' end) as modeflag,orgcode from vouchers where sid=%s", sid);
	if(ret)
	{
		LogPrint(VOUCHER_SEND_NODE, "sql error[%s]", mysql_error(_db.Handle()));
		return ret;
...
int CDboperate::dbo_get_sm_tasks(const char*sid,SCAN_INFO_T& si, vector<string>& vc_fname, vector<string>& vc_bname)
{
	...
	sprintf(strsql, "select fname,bname,pkgno,date,session,ip ,rotate,dpi,concat(right(pkgno,6),'01',right(accno,10)),vchno,vchtype,right(exchno,5),amount*100,imageid,sid,mainid,envid ,clkno,area,(case when modeflag then 'N' else 'O' end) as modeflag,orgcode from multis where sid=%ld", atol(sid));
