...
public:
	...
	virtual ~ZSocket()
            ...
            // SOCK_DGRAM
	int 	Create( int ST=AF_INET,int Type=SOCK_STREAM )
			{
	...
	SOCKET 	Accept( struct sockaddr *addr, int &addrlen )	{	return accept(m_hSocket,addr,(socklen_t *)&addrlen);	}
	...
	int 	RecvEx( char* buf, int buflen, int timeout = 50000 )
			{
				timeval tv;
