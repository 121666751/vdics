 ...
 *
 * 文件：	$Id: SerialPort.cpp,v 1.2 2008/03/30 09:51:03 Wu Exp $
 * 版本：	$Revision: 1.2 $
 * 网址：   www.tchzt.com.cn
...
//
void CSerialPort::CommThread(LPVOID pParam)
{
...
// start comm watching
//
BOOL CSerialPort::StartMonitoring(void *pCsp)
{
	m_Thread = _beginthread(CommThread,4096,pCsp);
    if( long(m_Thread) == -1 ) return FALSE;
...
//
BOOL CSerialPort::RestartMonitoring(void *pCsp)
{
	//TRACE("Thread resumed\n");
	//m_Thread->Resume();
	return StartMonitoring(pCsp);
}
