...
void __fastcall TdistForm::FormShow(TObject *Sender)
{
    ...
    int ret = FmDistMan->gdb.VQuery(512,"select distinct exchno from dists where date='%s' and session='%s' order by exchno ",FmDistMan->date,FmDistMan->session);
    if(ret)
    {
    ...
    FmDistMan->GetDS("date",date);
    FmDistMan->GetDS("session",session);
    txtDate->Text = date;
...
int TdistForm::flushData()
{
    ...
    str = str +cond;
    int ret = FmDistMan->gdb.Query(str.c_str());
    if(ret)
    {
         MessageBox(Handle,"查询出错","提示",MB_OK|MB_ICONINFORMATION);
         return ret;
   ...
   str = str +cond;
   ret = FmDistMan->gdb.Query(str.c_str());
   if(ret)
   {
        MessageBox(Handle,"查询出错","提示",MB_OK|MB_ICONINFORMATION);
        return ret;
...
//---------------------------------------------------------------------------
void __fastcall TdistForm::txtNetnoCloseUp(TObject *Sender)
{
    flushData();
}
//---------------------------------------------------------------------------
...
void __fastcall TdistForm::txtDetailDblClick(TObject *Sender)
{
    ...
    int ret = FmDistMan->gdb.VQuery(512,"select sid, accno,vchno,vchtype,amount ,mergestate,fname,ip ,0,date,session,isenv,modified,clkno from dists where sid=%d",sid);
    if(ret)
    {
        MessageBox(Handle,"查询出错","提示",MB_OK|MB_ICONINFORMATION);
        return;
...
void __fastcall TdistForm::txtNetnoDropDown(TObject *Sender)
{
    ...
    int ret = FmDistMan->gdb.Query("select distinct exchno from dists order by exchno");
    if(ret)
    {
        MessageBox(Handle,"查询出错","提示",MB_OK|MB_ICONINFORMATION);
        return;
