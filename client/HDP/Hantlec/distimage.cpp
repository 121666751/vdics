...
void __fastcall TdistFormImage::btnSaveClick(TObject *Sender)
{
   ...
   int ret =  FmDistMan->gdb.VQuery(512,"update dists set modified=1 ,accno='%s',vchno='%s',vchtype='%s',clkno='%s',isenv=%d where sid=%d",txtAccno->Text.c_str(),txtVchno->Text.c_str(),txtVchtype->Text.c_str(),g_sys_opt.c_str(),cbType->ItemIndex,curID);
   if(ret)
   {
