...
callback is valid.*/
#define VOPENDWG_STRING_CHECK(psz)	psz && psz[0] != VTEXTA(0)
...
#define	VOPENDWG_INIT_STATICS \
	...
	VSTRINGA VOpenDWG::m_pszInitFile =					NULL;		\
	VSTRINGA VOpenDWG::m_pszInitFileOffSet =			NULL;		\
...
public:
	...
	/** Return the string name of this layer. This method always returns
	an ASCII string, which will have to be converted for UNICODE use. See
	the VStringConverter class for help with this.*/
	VSTRING_CONSTA	Name() const
		{ return m_Layer.name; }
	...
	void			Name(VSTRING_CONST pszName)
	{
		...
		/* Copy string.*/
		VSTRNCPYA(	m_Layer.name,
					VStringConverter(pszName),
					VARRAY_SIZE(m_Layer.name) - 1);
	}
...
private:
	...
	void			SetLayerFlagBit(VCHARA	nBit,
									VBOOL	bSet)
	{
		if ( bSet )
			m_Layer.flag |= nBit;
...
public:
														...
														VFALSE)
	{
		...
		VLinkedListStringItem* pstrNew =
			VNEW VLinkedListStringItem(pszPath);
...
public:
	...
	static VBOOL	Init(	VBOOL			bInitialize =
												VTRUE,
							VSTRING_CONST	pszADINIT_DATFileName =
												NULL,
							VSHORT*			pnError =
												NULL)
	{
	...
	VOpenDWGLayer*	LayerAdd(VSTRING_CONST pszName)
	{
		...
		if ( !LayerFind(pszName) )
		{
			...
			/* Copy string.*/
			VSTRNCPYA(	layer.name,
						VStringConverter(pszName),
						VARRAY_SIZE(layer.name) - 1);
	...
	VOpenDWGLayer*	LayerFind(VSTRING_CONST pszName)
	{
		...
		while ( pLayer )
		{
			if ( VSTRCMP_NOCASE(pszName,
								VStringConverter(pLayer->Name())) == 0 )
			{
				break;
			}
			else
	...
	AutoCAD Release 14.*/
	VBOOL			NewDrawing(	AD_OBJHANDLE*	pStockObjHandle =
													NULL,
								AD_CLASS*		pClasses =
													NULL,
								VSHORT			nNumClasses =
									...
									VBOOL			bA2KObjects = VFALSE)
	{
		...
		if ( !CloseDrawing() )
		{
			...
			m_dbHandle = adLoadFile(
				(VVOIDPTR)(VSTRINGA)VStringConverter(pszFileName),
				AD_PRELOAD_ALL,
				(bA2KObjects) ? 1 : 0);
	...
	VBOOL			SaveDrawing(	VSTRING_CONST	pszFileName =
														NULL,
									VCHARA			nFileType =
														-1,
									VSHORT			nVersion =
	...
	VBOOL			ZoomNamedView(VSTRING_CONST pszName)
	{
		...
		for ( VLONG i = 0; i < nCount; i++ )
		{
			...
			{
				if ( VSTRCMP_NOCASE(VStringConverter(view.name),
									pszName) == 0 )
				{
...
protected:
	...
	VOpenDWGLayer*	InitLayerList()
	{
		...
		for ( VLONG i = 0;  i < nCount; i++)
		{
			...
			{
				VOpenDWGLayer* pLayer =
					VNEW VOpenDWGLayer(m_dbHandle, layer);
	...
	virtual void	OnBadFontFile(VSTRING_CONST pszFontFileName)
		{;}
	...
	names to make sure.*/
	virtual void	OnBadXREF(VSTRING_CONST pszFileName)
		{;}
	...
	only works on SHX type fonts, not TrueType fonts. If a new font is set
	as pszNewFontFileName, it must be ASCII, not UNICODE. the pszFontFileName
	is passed to this function as UNICODE when used, or ASCII otherwise.*/
	virtual void	OnExamineFontFile(	VSTRING_CONST	pszFontFileName,
										VSTRINGA		pszNewFontFileName)
	{
		...
		if (	s.FindFirst(VTEXT('|')) != -1 ||
				s.FindFirst(VTEXT(".ttf")) != -1 )
		{
			return;
		}
		...
		while ( pItem )
		{
			...
			{
				VSTRCPYA(pszNewFontFileName, VStringConverter(sFullPath));
				return;
			}
	...
	should be left as this to cause the XREF to be removed. If
	pszNewXREFFileName if modified, it must be done in ASCII, not UNICODE.*/
	virtual void	OnExamineXREF(	VSTRING_CONST	pszXREFFileName,
									VSTRINGA		pszNewXREFFileName)
	{
		if ( m_strFileName.IsNotEmpty() )
		{
			/* Make copy.*/
			VString sFullPath(m_strFileName);

			/* Terminate string at last file path.*/
			sFullPath.ReplaceLastChar(VFILE_PATH_SEP_CHAR);
			sFullPath += VFILE_PATH_SEP_CHAR;
			sFullPath += pszXREFFileName;

			if ( VDoesFileExist(sFullPath) )
			{
				VSTRCPYA(pszNewXREFFileName, VStringConverter(sFullPath));
				return;
			}
		}
	...
	virtual void	OnXREFOpenNotify(VSTRING_CONST pszFileName)
		{;}
	...
	static VSTRINGA				m_pszInitFile;
	static VSTRINGA				m_pszInitFileOffSet;
...
private:
	...
	static VSHORT	vOpenInitFile()
	{
		...
		if ( m_hInitFileMem )
		{
			m_pszInitFileOffSet = m_pszInitFile =
				(VSTRINGA)LockResource(m_hInitFileMem);
	...
	pointer by 1 char.*/
	static VCHARA	vReadInitFileChar()
	{
	...
	static void		vReadInitFileChars(	VSTRINGA	psz,
										VUSHORT		nNumChars)
	{
		VASSERT(psz)
		VASSERT(m_pszInitFileOffSet)
	...
	/** Examine font file.*/
	static void		vExamineFontFile(	VSTRINGA	pszFontFileName,
										VSTRINGA	pszNewFontFileName,
										VVOIDPTR	pUserData)
	{
		if ( VOPENDWG_STRING_CHECK(pszFontFileName) )
		{
			pszNewFontFileName[0] = VTEXTA('\0');

			if ( pUserData )
			{
				VOPENDWG_PTR(pUserData)->OnExamineFontFile(
					VStringConverter(pszFontFileName),
					pszNewFontFileName);
			}

			if ( pszNewFontFileName[0] == VTEXTA('\0') )
	...
	static void		vExamineXREF(	VSTRINGA	pszXREFFileName,
									VSTRINGA	pszNewXREFFileName,
									VVOIDPTR	pUserData)
	{
		if ( VOPENDWG_STRING_CHECK(pszXREFFileName) )
		{
			pszNewXREFFileName[0] = VTEXTA('\0');

			if ( pUserData )
			{
				VOPENDWG_PTR(pUserData)->OnExamineXREF(
					VStringConverter(pszXREFFileName),
					pszNewXREFFileName);
			}
		}
	}
	...
	static void		vXREFOpenNotify(	VSTRINGA	pszFileName,
										VVOIDPTR	pUserData)
	{
		if ( pUserData )
			VOPENDWG_PTR(pUserData)->
				OnXREFOpenNotify(VStringConverter(pszFileName));
	}
...
/* Include extra support if doing a Win32 GUI application.*/
		...
		public:
			...
			/** Set the active pen and brush to the color specified by
		...
		protected:
			...
			public:
				...
				void			SetRecycler(
					VOpenDWGResourceRecycler* pResourceRecycler)
