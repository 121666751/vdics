...
/** VListView provides extended wrapping of the Win32 listview common
control with many enhancements. In addition to being a listview, this
class can be used to implement a generic image viewer using the
GetBackImage() and SetBackImage() methods. It can view graphic files
that are on the local system as well as those specified with Internet
URL's.*/
...
public:
	...
	void			BestFitColumns(	VINT nStartColumn =
											0,
									VINT nEndColumn =
											-1) const
	{
	...
	will be retrieved.*/
	void			BestFitColumnsEx(	VINT	nStartColumn =
													0,
										VINT	nEndColumn =
													-1,
										VSTRING	pszBuffer =
													...
													0) const
	{
		...
		VCHAR sz[VWCL_MAX_SINGLE_LISTVIEW_ITEM_LENGTH];
		...
		VCHAR				szHeader[VWCL_MAX_SINGLE_LISTVIEW_ITEM_LENGTH];
		HD_ITEM				header;
	...
	extended style sets.*/
	void			Checked(VINT	nIndex,
							VBOOL	bChecked) const
	{
		ItemState(	nIndex,
					LVIS_STATEIMAGEMASK,
	...
	before sending the message, as required by the control.*/
	...
	/** Get the current background image. The LVBKIMAGE specified by the
	image parameter will be initialized with requested information on
	success. You should populate the image.ulFlags to set the information
	you wish to retrieve. LVBKIMAGE contains information about the background
	image of a list view control. This structure is used for both setting and
	retrieving background image information. The members of LVBKIMAGE are
	defined as:

	ulFlags - One or more of the following flags. The LVBKIF_SOURCE_MASK
	value can be used to mask off all but the source flags. The
	LVBKIF_STYLE_MASK value can be used to mask off all but the style
	flags. LVBKIF_SOURCE_NONE means the list view control has no background
	image.LVBKIF_SOURCE_HBITMAP is not currently implemented.
	LVBKIF_SOURCE_URL means the pszImage member contains the URL of the
	background image. LVBKIF_STYLE_NORMAL means the background image is
	displayed normally. LVBKIF_STYLE_TILE means the background image will
	be tiled to fill the entire background of the control.

	hbm - Not currently used.

	pszImage - Address of a NULL-terminated string that contains the URL of
	the background image. This member is only valid if the LVBKIF_SOURCE_URL
	flag is set in ulFlags. This member must be initialized to point to the
	buffer that contains or receives the text before sending the message. The
	URL can be a normal Internet URL or a local file, but has to be in
	standard URL syntax (file:///C:/somefolder/somefile.gif). The function
	UrlCreateFromPath() is helpful.

	cchImageMax - Size of the buffer at the address in pszImage. If
	information is being sent to the control, this member is ignored.

	xOffsetPercent - Percentage of the control's client area that the image
	should be offset horizontally. For example, at 0 percent, the image will
	be displayed against the left edge of the control's client area. At 50
	percent, the image will be displayed horizontally centered in the
	control's client area. At 100 percent, the image will be displayed against
	the right edge of the control's client area. This member is only valid
	when LVBKIF_STYLE_NORMAL is specified in ulFlags.

	yOffsetPercent - Percentage of the control's client area that the image
	should be offset vertically. For example, at 0 percent, the image will
	be displayed against the top edge of the control's client area. At 50
	percent, the image will be displayed vertically centered in the
	control's client area. At 100 percent, the image will be displayed
	against the bottom edge of the control's client area. This member is
	only valid when LVBKIF_STYLE_NORMAL is specified in ulFlags.

	See the SetBackImage() method for more information. Returns VTRUE on
	success, VFALSE on failure.*/
	VBOOL 			GetBackImage(LVBKIMAGE& image) const
		{ return VWINDOW_SMB2(LVM_GETBKIMAGE, 0, &image); }
								...
								VINT		nColumn) const
	{
			...
			{
				VASSERT(column.pszText)
				VASSERT(column.cchTextMax)
			}
		#endif
	...
	VBOOL			GetItem(LV_ITEM& item) const
	{
			...
			{
				VASSERT(item.pszText)
				VASSERT(item.cchTextMax)
			}
		#endif
	...
	/** Return a pointer to the header control, or NULL if there is not one.
	The header control is only valid when using the Report mode of the
	listview, which is controlled during Create() using the LVS_REPORT
	style. If the style LVS_NOCOLUMNHEADER is also used, the header control
	will not be created. This header control object is owned by the listview
	and must never be deleted by calling code.*/
	VHeader*		GetHeader() const
	{
		if ( m_wndHeader.GetHandle() )
			return (VHeader*)&m_wndHeader;
	...
	will be used.*/
	void			InsertColumnsID(VUINT*		pStringIDArray,
									VINT		nFormat =
													-1,
									VINT		nColumn =
													-1,
	...
	un-selected.*/
	void			Selection(	VINT	nStartIndex =
											0,
								VINT	nEndIndex =
											-1,
								VBOOL	bSelect =
	...
	/** Set the background image. Returns VTRUE on success. VFALSE on
	failure. Because the list view control uses OLE COM to manipulate the
	background images, the application  must call CoInitialize() or
	OleInitialize() before calling this function. VWCL has a class named
	VActiveXInitializer that can perform this for you. See the
	GetBackImage() method for more information about the proper use of the
	LVBKIMAGE structure in the image parameter.*/
	VBOOL			SetBackImage(LVBKIMAGE& image) const
		{ return VWINDOW_SMB2(LVM_SETBKIMAGE, 0, &image); }
	...
	VFALSE on failure.*/
	VBOOL			SetColumn(	LV_COLUMN&	column,
								VINT		nColumn) const
...
protected:
	/** Override attached the header control to the internal header member
	if the listview is in report mode. See GetHeader() for more information.*/
	virtual VBOOL	SetupWindow()
	{
		if ( VCommonControl::SetupWindow() )
		{
			VBitmask style(Style());

			/* Must be report mode with column headers.*/
			if (	style.IsSet(LVS_REPORT) &&
					style.IsNotSet(LVS_NOCOLUMNHEADER) )
			{
				HWND hWnd = (HWND)VWINDOW_SM0(LVM_GETHEADER);
				VASSERT(hWnd)

				if ( hWnd )
				{
					/* If this control is attached, do the same
					for the header. Otherwise, just wrap the header.*/
					if ( IsAttached() )
						m_wndHeader.Attach(hWnd);
					else
						m_wndHeader.GetHandle() = hWnd;
				}
			}
