...
public:
	...
	virtual void		BeginDragDrop(	HTREEITEM	hItemDrag =
														NULL,
										VBOOL		bSelect =
														VTRUE)
	{
															...
															NULL)
	{
		...
		if ( sFind.IsNotEmpty() )
		{
			VDelimitedString	ds(sFind, pszSeparator[0]);
			VSTRING				pszOffSet;
			hItem = hParent;
	...
	VFALSE otherwise.*/
	VBOOL				CheckState(	HTREEITEM	hItem =
													NULL,
									VUINT		nState =
													TVIS_EXPANDED) const
	{
	...
	virtual VBOOL		Collapse(	HTREEITEM	hItem =
													NULL,
									VUINT		nFlags =
													TVE_COLLAPSE)
	{
