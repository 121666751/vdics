...
public:
	...
	VUINT		Browse(VSTRING_CONST pszURL)
	{
		...
		if ( !pszServiceName )
		{
					...
					strBrowserPath.IsNotEmpty() )
			{
				...
				if ( strBrowserPath.FindFirst(
					VWEBHYPERLINK_NETSCAPE_STRING) != -1 )
	...
	VUINT		BrowseDDE(	VSTRING_CONST	pszURL,
							...
							VBOOL			bRecurseOnFailure =
												VTRUE)
	{
		/* These must be known!*/
		VASSERT(VSTRLEN_CHECK(pszURL))
		VASSERT(VSTRLEN_CHECK(pszServiceName))
		...
		{
			#ifdef VWCL_UNICODE
				VINT nCodePage = CP_WINUNICODE;
			#else
				VINT nCodePage = CP_WINANSI;
			#endif
	...
	static VBOOL	IsWindowNetscape(HWND hWnd)
	{
		...
		VString s;
		VWindowString(s, hWindow);

		if (	s.UpperCase() &&
				s.FindFirst(VWEBHYPERLINK_NETSCAPE_STRING) != -1 )
			return VTRUE;
