...
public:
												...
												VTRUE)
	{
		...
		knows this!*/
			...
			{
				#ifndef VTOOLBAR
					MessageBox(VTEXT("It looks like you want a toolbar for your main window. Please include common_controls/vtoolbar.hpp before including vmainwindow.hpp."), VWCL_DEBUG_MESSAGE_STRING);
				#endif
			}
			...
			{
				#ifndef VSTATUSBAR
					MessageBox(VTEXT("It looks like you want a statusbar for your main window. Please include common_controls/vstatusbar.hpp before including vmainwindow.hpp."), VWCL_DEBUG_MESSAGE_STRING);
				#endif
			}
		#endif
		...
		GetOptions().Save();
		GetOptions().Append(OPTION_DONT_CALL_SETUP_WINDOW);
									...
									(VUINT)m_Menu.GetHandle()) )
		{
			...
			if (	GetOptions().IsSet(OPTION_DONT_CALL_SETUP_WINDOW) ||
					SetupWindow() )
			{
	...
	displayed, before exit, and makes sure the main window is visible
	before entering the message pump.*/
	VWPARAM			EnterMessageLoop()
	{
		...
		/* Enter message loop.*/
		VWPARAM nResult = VWindow::EnterMessageLoop();

		/* Close WinHelp if started.*/
		WinHelp(GetHandle(), NULL, HELP_QUIT, 0);
			...
			switch ( nmhdr.code )
			{
					...
					case TTN_NEEDTEXT:
					{
						...
						if ( m_pToolBar )
						{
							...
							VString s(VResourceString(pToolTip->hdr.idFrom));
							
							if ( s.IsNotEmpty() )
							{
								/* Tooltip text starts after newline
								char, or first char if not found.*/
								VINT nStartTip = s.FindFirst(VTEXT('\n'));

								VSTRNCPY(	pToolTip->szText,
											s.String() + VMAX(nStartTip + 1, 0),
											80);

								#ifdef VSTATUSBAR
									/* Set string in pane 0.*/
									if ( nStartTip != -1 )
										s.CharAt(nStartTip) = VTEXT('\0');
									UpdateStatusBar(s);
								#endif
							}
						}
			...
			if ( m_pStatusBar )
			{
				switch ( nMessage )
				{
					...
					case WM_MENUSELECT:
					{
						...
						VMenu	menuSelected((HMENU)lParam, VFALSE);

						/* We will hold the string to display here.*/
						VString strStatus;

						/* Determine if there is something to do.*/
						VBOOL bIsCommand = VTRUE;

						/* Was a menu closed?*/
						if ( nFlags & 0xffff && !menuSelected.GetHandle() )
							bIsCommand = VFALSE;
						/* Is it opening a sub-menu?*/
						...
						if ( bIsCommand )
						{
							...
							{
								if ( menuSelected.GetItemString(nCommandID,
																strStatus) )
								{
									/* Remove ampersand.*/
									VINT nAmp =
										strStatus.FindFirst(VTEXT('&'));
							...
							else if ( strStatus.String(
								VResourceString(nCommandID)) )
							{
								strStatus.ReplaceFirstChar(VTEXT('\n'));
							}
						}

						/* Truncate string at tab. Remainder is for hotkeys.*/
						strStatus.ReplaceFirstChar(VTEXT('\t'));
