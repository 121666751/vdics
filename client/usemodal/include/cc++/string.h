...
public:
	 ...
	 * @param data to input.
	 */
	String(size_t count, const char *fmt, ...);
#else
        /**
	...
	void replace(size_t start, size_t len, const char *text, size_t count = 0);
         ...
         */
	void replace(size_t start, size_t len, const String &s);
	 ...
	 */
	inline size_t find(unsigned instance, const char *cp, size_t offset = 0, size_t len = 0) const
		{return find(cp, offset, len, instance);};
         ...
         * @param offset offset to start at.
         */
	inline size_t find(unsigned instance, const String &s, size_t offset = 0) const
		{return find(s, offset, instance);};
	 ...
	 * @param idx index or offset to use.
	 */
	 ...
	 */
	friend __EXPORT int strprintf(String &str, size_t size, const char *fmt, ...);
#endif
