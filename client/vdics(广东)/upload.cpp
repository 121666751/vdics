...
int TFormTrans::flushData()
{
    ...
    {
    //------------------------------------------------------------------------20150511
        if(txtExchno->Text.Length() != 6)
        {
                cond = cond + AnsiString(" and pkgno = '")+txtExchno->Text.c_str()+"'";
        }
        else
    //------------------------------------------------------------------------20150511
...
//--------------------------------------------------------------20150511
                if(txtExchno->ItemIndex != 0)
                        li->SubItems->Insert(0,txtExchno->Text);
                else
//--------------------------------------------------------------20150511
...
int TFormTrans::flushExchno()
{
    ...
    int ret = FmIC->gdb.VQuery(512,"select distinct exchno,pkgno from vouchers where date='%s' and session='%s' and area='%s' order by exchno",txtDate->Text.c_str(),txtSession->Text.c_str(),FmIC->m_area.c_str());
