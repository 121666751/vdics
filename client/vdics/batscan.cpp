...
int TFormScan::saveToDb()
{
    ...
    while( pos != imgList.end())
	{
        if(_scanType == 0 )
        {
            sql.sprintf("insert into vouchers(fname,bname,ip,rotate,exchno,pkgno,date,session,dpi,scantime,timestamp,mergestate,area,state,vchtype,modeflag,clkno,orgcode) values ('%s','%s','%s',%d,'%s','%s','%s','%s',%d,now(),now(),%d,'%s','%d','%s',%d,'%s','%s')", pos->fname,pos->bname,pos->ip,pos->rotate,pos->pkgno ,pk,pos->date,pos->session,pos->dpi,mt,Area.c_str(),state,_vchType.c_str(),newmode,g_sys_opt.c_str(),g_sys_org.c_str());
        }
        else if(_scanType == 1 )
        {
            sql.sprintf("insert into  multis(fname,bname,ip,rotate,exchno,pkgno,date,session,dpi,scantime,timestamp,multiflag,mergestate,area,clkno,orgcode) values ('%s','%s','%s',%d,'%s','%s','%s','%s',%d,now(),now(),%d,%d,'%s','%s','%s')", pos->fname,pos->bname,pos->ip,pos->rotate,pos->pkgno ,pk,pos->date,pos->session,pos->dpi,single,mt,Area.c_str(),g_sys_opt.c_str(),g_sys_org.c_str());
        }
