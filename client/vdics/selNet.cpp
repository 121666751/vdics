...
void __fastcall TFmSelNet::btnOkClick(TObject *Sender)
{
    ...
    if(_type == 0 )
    {
        ret=FmIC->gdb.VQuery(512,"select right(left(state,3),1) from bocctrl where  exchno='%s' and area='%s' and date='%s' and session='%s'" ,txtPkgNo->Text.c_str(),g_sys_area.c_str(),FormScan->date,FormScan->session);
    }
    else if(_type == 1 )
    {
        ret=FmIC->gdb.VQuery(512,"select right(left(state,4),1) from bocctrl where  exchno='%s' and area='%s' and date='%s' and session='%s'" ,txtPkgNo->Text.c_str(),g_sys_area.c_str(),FormScan->date,FormScan->session);
    }
    else if(_type == 2 )
    {
        ret=FmIC->gdb.VQuery(512,"select right(left(state,5),1) from bocctrl where  exchno='%s' and area='%s' and date='%s' and session='%s'" ,txtPkgNo->Text.c_str(),g_sys_area.c_str(),FormScan->date,FormScan->session);
    }
    else
    {
        ret=FmIC->gdb.VQuery(512,"select right(left(state,6),1) from bocctrl where  exchno='%s' and area='%s' and date='%s' and session='%s'" ,txtPkgNo->Text.c_str(),g_sys_area.c_str(),FormScan->date,FormScan->session);
    }
    if(ret)
    {
    ...
    if(_type == 0 )
        FmIC->gdb.VQuery(512,"update bocctrl set state=concat(left(state,2),'2',right(state,'13')) where exchno='%s' and area='%s' and date='%s' and session='%s'",txtPkgNo->Text.c_str(),g_sys_area.c_str(),FormScan->date,FormScan->session);
    else if(_type == 1 )
        FmIC->gdb.VQuery(512,"update bocctrl set state=concat(left(state,3),'2',right(state,'12')) where exchno='%s' and area='%s' and date='%s' and session='%s'",txtPkgNo->Text.c_str(),g_sys_area.c_str(),FormScan->date,FormScan->session);
    else if(_type == 2 )
        FmIC->gdb.VQuery(512,"update bocctrl set state=concat(left(state,4),'2',right(state,'11')) where exchno='%s' and area='%s' and date='%s' and session='%s'",txtPkgNo->Text.c_str(),g_sys_area.c_str(),FormScan->date,FormScan->session);
    else
        FmIC->gdb.VQuery(512,"update bocctrl set state=concat(left(state,5),'2',right(state,'10')) where exchno='%s' and area='%s' and date='%s' and session='%s'",txtPkgNo->Text.c_str(),g_sys_area.c_str(),FormScan->date,FormScan->session);
