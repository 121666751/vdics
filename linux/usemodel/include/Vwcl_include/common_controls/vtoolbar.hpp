...
public:
	...
	/** Hides the specified button in the toolbar. Returns VTRUE if
	successful, or VFALSE otherwise. nButtonID is the command identifier of
	the button to hide.*/
	VBOOL				HideButton(VWPARAM nButtonID)
		{ return ShowButton(nButtonID, VFALSE); }
	...
	number requested.*/
	void				Rows(	VUINT	nNumRows,
								VBOOL	bLarger,
								VRect&	r) const
	{
		VASSERT(nNumRows >= 1)
		VASSERT(nNumRows <= GetButtonCount())
	...
	VBOOL				SetButtonSize(	VWORD nWidth,
										VWORD nHeight) const
	...
	VWORD				State(VWPARAM nButtonID) const
		{ return (VWORD)VWINDOW_SM1(TB_GETSTATE, nButtonID); }
...
protected:
	...
	virtual VBOOL		OnPaint(VDC& dc)
	{
		...
		VCommonControl::WindowProc(	GetHandle(),
									WM_PAINT,
