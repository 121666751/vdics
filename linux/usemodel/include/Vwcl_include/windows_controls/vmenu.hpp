...
public:
	...
	is -1.*/
	VUINT	GetDefaultItem(	VUINT gmdiFlags =
									GMDI_USEDISABLED,
							VBOOL bByPos =
									VFALSE) const
	{
	...
	/** Determine the number of items in the specified menu.*/
	VUINT	GetItemCount() const
	{
		VASSERT(GetSafeHandle())
		
		VINT nCount = GetMenuItemCount(m_hMenu);
	...
	VUINT	GetItemString(	VUINT		nIDItem,
							...
							VUINT		nFlag =
											MF_BYCOMMAND) const
	{
		/* Determine length and allocate memory for it.*/
		VMemoryBlock	memBlock;
		VUINT nLength = GetItemString(nIDItem, NULL, 0, nFlag);

		if ( nLength )
		{
			/* Add one character more in case this item has an & in it.*/
			nLength++;

			/* Allocate sufficient memory.*/
			if ( memBlock.AllocForString(nLength) )
			{
				if ( GetItemString(	nIDItem,
