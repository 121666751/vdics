...
public:
	...
	VNTService(	VSTRING_CONST	pszServiceName =
									NULL,
		...
		if ( hSCM )
		{
			if ( bInstall )
			{
				...
				if ( !pszFullPath )
				{
					...
					ps.LoadModulePath(VFALSE, VFALSE);
				}
				else
					sPath = pszFullPath;
	...
	VBOOL					ProcessCommandLine()
	{
		...
		if ( VGetARGC() > 0 )
		{
			...
			if ( theApp.IsCommandLineOption(VTEXTA("I")) )
			{
				Install(VTRUE);
				bServiceShouldStart = VFALSE;
			}
			else if ( theApp.IsCommandLineOption(VTEXTA("U")) )
			{
				Install(VFALSE);
				bServiceShouldStart = VFALSE;
			}
			else if ( theApp.IsCommandLineOption(VTEXTA("?")) )
			{
				OnHelp();
				bServiceShouldStart = VFALSE;
			}
...
protected:
	...
	virtual VBOOL			OnCommandLineParameter(VSTRING_CONSTA pszCommand)
		{ return VTRUE; }
