...
void main() {
	...
	int ret = db.Open("2.1.1.38","root","","testdb");
	if( ret ) return;

	// send Query command
	int ret = db.Query("select A,B from testtb where a=3") or
	int ret = db.VQuery(512,"select A,B from testtb where a=%d and d='%s'",i,str);
	if( ret ) return;
...
inline int MyDb::Open(const char* Host, const char* UserName, const char* Password, const char* DatabaseName)
{
	...
	if( mysql_real_connect(m_hMysql,Host,UserName,Password,DatabaseName,NULL,NULL,NULL) )
		m_bConnected = true;
	else
		m_bConnected = false;
...
inline int MyDb::Query(const char* Command, int CmdLen)
{
	...
	if(ret) OutputDebugString(mysql_error(m_hMysql));
...
inline int MyDb::VQuery(unsigned int BufferLen, const char* fmt, ...)
{
    ...
    try{
        va_list ap;
        va_start(ap,fmt);
        _vsnprintf(cmd_ptr,BufferLen,fmt,ap);
        va_end(ap);
