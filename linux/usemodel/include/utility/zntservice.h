...
/** ZNTService encapuslates writing services for Windows NT. Unlike other
...
and shut down. Derived from ZNTEventLog, services derived from this class
...
ZNTService* ZNTService::m_pThis = NULL;

See the ZNTServiceTest sample application for more information. Because of
...
public:
	...
	ZNTservice.mc file, which is included in the "resources" source tree.
	...
	ZNTservice.rc. EM_USER_STRING is a special case. Use it when you want
	...
	the location of Event Log message resources. See ZNTEventLog for very
	important information on the hMessageResources parameter.*/
	ZNTService(	const char*	pszServiceName,
				DWORD			nControlsAccepted =
									SERVICE_ACCEPT_STOP |
	...
	to NULL. Override this function, and when bInstall is TRUE, get the
	...
	CreateService() API function. Returns TRUE on success, FALSE on
	failure.*/
	virtual BOOL			Install(	BOOL			bInstall =
															TRUE,
										DWORD			nStartType =
															SERVICE_AUTO_START,
										const char*	pszFullPath =
															NULL,
										const char*	pszLogonAs =
															NULL,
										const char*	pszLogonPassword =
															NULL,
										BOOL			bStartAfterInstall =
															TRUE,
										BOOL			bSilentMode =
															FALSE)
	{
		...
		if ( hSCM )
		{
			if ( bInstall )
			{
				...
				Install(FALSE, 0, NULL, NULL, NULL, FALSE, TRUE);
...
//						reg.DeleteSubKeyNT(m_strServiceName);
	...
	BOOL					IsInstalled() const
	{
		...
		if ( hSCM )
		{
			...
			hService =
				OpenService(hSCM, m_strServiceName.c_str(), SERVICE_QUERY_CONFIG);
	...
	TRUE. Otherwise, it means the user wanted to only install or un-install
	the service, or show text mode help. Command line parameters that this
	function understands are as follows:

	-I = Install the Service. Calls the Install(TRUE) virtual function.

	-U = Uninstall the Service. Calls the Install(FALSE) virtual function.
	...
	{
		BOOL bServiceShouldStart = TRUE;

		/* See if at least 1 command line param is present.*/
		if ( __argc > 1 )
		{
			std::string opt = __argv[1];

			if ( opt == "-I" )
			{
				Install(TRUE);
				bServiceShouldStart = FALSE;
			}
			else if ( opt == "-U" )
			{
				Install(FALSE);
...
//				}
	...
	Log and base class. Return return value is TRUE as long as Service
	Name string was allocated. See ZNTEventLog for very important
	information on the hMessageResources parameter. If pszServiceName is
	NULL, VGetAppTitle() will be used. Returns TRUE on success, FALSE on
	failure. If the ZNTEventLog base class has already been initialized
	with a previous call to ZNTEventLog::Register() this function will not
	attempt to re-register. It will use the previously registered event
	handle to log messages.*/
	BOOL					ServiceName(const char*	pszServiceName,
										HINSTANCE		hMessageResources =
															NULL)
	{
		/* Save name.*/
...
//			Register(m_strServiceName, hMessageResources);
	...
	BOOL					Start()
	{
		...
		//LogEvent(EM_WAITING_FOR_SCM, FALSE);
		char svcname[256];
		strcpy(svcname, m_strServiceName.c_str());
		...
		return (StartServiceCtrlDispatcher(st)) ? TRUE : FALSE;
	}

protected:
    /** Called by base class when an unknown argument is passed on the
	command line. Return TRUE to have ProcessCommandLine() return TRUE to
	start the service, or return FALSE to return FALSE from
	ProcessCommandLine() and not start the service.*/
	virtual BOOL			OnCommandLineParameter(const char* pszCommand)
		{ return TRUE; }
	...
	fails. If bSilentMode is TRUE it means the function was invoked
	...
	virtual void			OnInstall(	BOOL bSuccess,
										BOOL bSilentMode)
		{;}
	...
	or fails. If bSilentMode is TRUE it means the function was invoked
	internally (like un-install before a fresh install), and you probably
	don't want to notify the user because they could get confused by
	conflicting messages.*/
	virtual void			OnUnInstall(	BOOL bSuccess,
											BOOL bSilentMode)
		{;}
	...
	virtual DWORD			OnContinue(	DWORD nCurrentState,
										DWORD nProposedState)
    ...
    virtual DWORD			OnPause(	DWORD nCurrentState,
										DWORD nProposedState)
		{ return nProposedState; }
	...
	being in another state. The default implementation of this function
	sets the new state to the proposed state.*/
    virtual DWORD			OnShutdown(	DWORD nCurrentState,
										DWORD nProposedState)
		{ return nProposedState; }
	...
	being in another state. The default implementation of this function
	sets the new state to the proposed state.*/
	virtual DWORD			OnStop(	DWORD nCurrentState,
									DWORD nProposed)
	{
    ...
    virtual void			OnUserControl(DWORD nOpcode)
		{;}

	/** Initial entry point passed to Service Control Manager.*/
	static void WINAPI		ServiceMain(	DWORD nArgc,
											char** pszArgv)
	{
		...
		m_pThis->m_hServiceStatus =
			RegisterServiceCtrlHandler(m_pThis->m_strServiceName.c_str(), SCMHandler);
		...
		//else
			//m_pThis->LogEvent(EM_HANDLER_FUNCTION_FAILURE, TRUE);
	}
	...
	static void WINAPI		SCMHandler(DWORD nOpcode)
	{
		/* Save the current state.*/
		DWORD nNewState = m_pThis->m_Status.dwCurrentState;
	...
	DWORD					m_nControlsAccepted;
	SERVICE_STATUS_HANDLE	m_hServiceStatus;
	SERVICE_STATUS			m_Status;
	static ZNTService*		m_pThis;
};
